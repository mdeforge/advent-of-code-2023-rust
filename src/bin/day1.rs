...
fn find_first_num_char(input: &str, search_backwards: bool) -> Option<(char, u32)> {
    ...
    // find both the first number and its position...
    for (index, character) in line.chars().enumerate() {
        if character.is_digit(10) {
            return Some((character, index as u32));
        }
    }
...
fn find_first_num_word(input: &str, search_backwards: bool) -> Option<(char, u32)> {
    ...
    let mut best_find = String::default();
    let mut best_find_pos = 0;
    let mut found_at_least_one = false;

    // TODO(mdeforge): Maybe NUMBER_LIST should not be a global const, maybe it should be passed in
    // TODO(mdeforge): Consider breaking some of the work in here into a different function
    for word in NUMBER_LIST {
        // if search_backwards is true, flip the word too!
        let mut oriented_word = word.to_string();
        if search_backwards {
            oriented_word = word.chars().rev().collect::<String>();
        }

        let word_pos = line.find(&oriented_word);
        if word_pos.is_some() {
            if !found_at_least_one {
                best_find = word.to_string();
                best_find_pos = word_pos.unwrap();
                found_at_least_one = true;
            } else {
                if word_pos.unwrap() < best_find_pos {
                    best_find = word.to_string();
                    best_find_pos = word_pos.unwrap();
                }
            }
    ...
    if found_at_least_one {
        let num_char = word_to_char(&best_find).unwrap();
        return Some((num_char, best_find_pos as u32));
    }

    None
}

fn find_first_occurrence(input: &str, search_backwards: bool) -> Option<char> {
    // find both the first number and its position...
    let first_num_char = find_first_num_char(input, search_backwards);

    // ...and the first word and its position
    let first_word_char = find_first_num_word(input, search_backwards);

    // Find the first occurence, be it a number or word
    if first_num_char.is_some() && first_word_char.is_some() {
        // We found both, so find which one came first
        if first_num_char.unwrap().1 < first_word_char.unwrap().1 {
            return Some(first_num_char.unwrap().0);
        } else {
            return Some(first_word_char.unwrap().0);
        }
    } else if first_num_char.is_some() {
        return Some(first_num_char.unwrap().0);
    } else if first_word_char.is_some() {
        return Some(first_word_char.unwrap().0);
    } else {
        return None;
    }
...
fn find_calibration_value(line: &str) -> Result<u32, std::num::ParseIntError> {
    let first = find_first_occurrence(line, false);
    let last = find_first_occurrence(line, true);
...
fn main() {
    ...
    for puzzle in puzzle_input {
        sum = sum + find_calibration_value(puzzle).unwrap();
    }
...
mod tests {
    ...
    #[case("four", '4')]
    #[case("nine", '9')]
    fn test_word_to_char(#[case] input_word: &str, #[case] expected_char: char) {
        assert_eq!(expected_char, word_to_char(input_word).unwrap());
    }
    ...
    #[rstest]
    #[case("two1nine", Some('2'))]
    fn test_find_first_occurrence_test_first(#[case] input: &str, #[case] expected: Option<char>) {
        assert_eq!(expected, find_first_occurrence(input, false))
    }

    #[rstest]
    #[case("two1nine", Some('9'))]
    fn test_find_first_occurrence_test_last(#[case] input: &str, #[case] expected: Option<char>) {
        assert_eq!(expected, find_first_occurrence(input, true))
    }
    ...
    fn test_find_calibration(#[case] input: &str, #[case] expected: u32) {
        assert_eq!(expected, find_calibration_value(input).unwrap())
    }
}
